...
t_mean_zonal    = nanmean(pt_mean,3);
...
for x = 1:(length(models)-1)
    ...
    'ylim',[0 5500],'ytick',0:500:5500,'yticklabel',{'0','500','1000','1500','2000','2500','3000','3500','4000','4500','5000','5500'},'yminort','on', ...
    'xlim',[-90 90],'xtick',-90:20:90,'xticklabel',{'-90','-70','-50','-30','-10','10','30','50','70','90'},'xminort','on');    
    export_fig([outData,'so/figs/',datestr(now,'yymmdd'),'_',tmp1name],'-png')
    close all
    clear handle ax1 ax2 hh1 tmp1 ind tmp1name
    % Plot model 2
    close all, handle = figure('units','centimeters','visible','off','color','w'); set(0,'CurrentFigure',handle)
    ...
    'ylim',[0 5500],'ytick',0:500:5500,'yticklabel',{'0','500','1000','1500','2000','2500','3000','3500','4000','4500','5000','5500'},'yminort','on', ...
    'xlim',[-90 90],'xtick',-90:20:90,'xticklabel',{'-90','-70','-50','-30','-10','10','30','50','70','90'},'xminort','on');    
    export_fig([outData,'so/figs/',datestr(now,'yymmdd'),'_',tmp2name],'-png');
    close all
    clear handle ax1 ax2 hh1 tmp2 tmp2name
...
set(ax2,'Tickdir','out','fontsize',fonts_ax,'layer','top','box','on', ...
    'ylim',[0 5500],'ytick',0:500:5500,'yticklabel',{'0','500','1000','1500','2000','2500','3000','3500','4000','4500','5000','5500'},'yminort','on', ...
    'xlim',[-90 90],'xtick',-90:20:90,'xticklabel',{'-90','-70','-50','-30','-10','10','30','50','70','90'},'xminort','on');
export_fig([outDir,datestr(now,'yymmdd'),'_CMIP5_so_mean'],'-png')
...
%% Save WOA09 and CMIP5 ensemble matrices to file
so_woa09_mean = s_mean; clear s_mean
thetao_woa09_mean = pt_mean; clear pt_mean
so_cmip5_mean = so_mean; clear so_mean
thetao_cmip5_mean = thetao_mean; clear thetao_mean
save([outDir,datestr(now,'yymmdd'),'_CMIP5andWOA09_thetaoAndso.mat'],'so_woa09_mean','thetao_woa09_mean','so_cmip5_mean','thetao_cmip5_mean', ...
                                                                      's_mean_zonal','so_mean_zonal','t_mean_zonal','thetao_mean_zonal')

%% Figure 9.13 - Temperature and salinity clim vs WOA09
% Determine depth split
depth1 = find(t_depth == 1000);

% Create anomaly
thetao_mean_anom_zonal = thetao_mean_zonal - t_mean_zonal;
so_mean_anom_zonal = so_mean_zonal - s_mean_zonal;

% Do thetao global
close all, handle = figure('units','centimeters','visible','on','color','w'); set(0,'CurrentFigure',handle)
...
contour(t_lat,t_depth(1:depth1),t_mean_zonal(1:depth1,:),[2.5 7.5 12.5 17.5 22.5 27.5],'k')
[c,h] = contour(t_lat,t_depth(1:depth1),t_mean_zonal(1:depth1,:),0:5:30,'k','linewidth',2);
...
set(h,'linestyle','none'); hold all; clear h
axis ij, caxis([-1 1]*ptscale(1)), clmap(27), hold all
contour(t_lat,t_depth(depth1:end),t_mean_zonal(depth1:end,:),[2.5 7.5 12.5 17.5 22.5 27.5],'k')
[c,h] = contour(t_lat,t_depth(depth1:end),t_mean_zonal(depth1:end,:),0:5:30,'k','linewidth',2);
clabel(c,h,'LabelSpacing',200,'fontsize',fonts_c,'fontweight','bold','color','k')
...
set(hh3,'Position',[0.0750 0.048 0.41 0.015],'fontsize',fonts);
set(ax2,'Position',[0.535 0.58 0.45 0.40]);
set(ax4,'Position',[0.535 0.17 0.45 0.40]);
set(hh4,'Position',[0.555 0.048 0.410 0.015],'fontsize',fonts);
...
set(xlab3,'Position',[0 5700 1.0001]);
set(xlab4,'Position',[0 5700 1.0001]);

% Print to file
if isunix
    export_fig([outDir,datestr(now,'yymmdd'),'_AR5_Fig9p13_CMIP5minusWOA09_thetaoAndso'],'-eps')
elseif ispc
    export_fig([outDir,datestr(now,'yymmdd'),'_AR5_Fig9p13_CMIP5minusWOA09_thetaoAndso_win'],'-eps')
end
